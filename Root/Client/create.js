...
const flashcardForm = document.querySelector(
  ".flashcard-creation-container form"
);
...
publishButton.addEventListener("click", () => {
  ...
  for (let i = 0; i < flashcardsTerms.length; i++) {
    flashcardArray.push(
      new Flashcard(
        flashcardsTerms[i].value,
        flashcardsDefs[i].value,
        flashcardsValues[i].value
      )
    );
  }
  ...
  for (let card of newSet.flashcards) {
    questionsToUpload.push({
      question: card.term,
      answer: card.definition,
      pointValue: card.value,
    });
  }
  createQuestionSet(questionsToUpload, newSet.name, () => {
    console.log("sadf");
  });
